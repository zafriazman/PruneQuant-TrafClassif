...
class CNN1D_TrafficClassification_with_auxiliary(nn.Module):
    ...
    def __init__(self, input_ch, num_classes):
        ...
        self.conv1 = nn.Conv1d(in_channels=input_ch, out_channels=16, kernel_size=3, stride=1, padding='same')
        ...
        self.conv2 = nn.Conv1d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding='same')
        self.bn2 = nn.BatchNorm1d(16)
        self.pool2 = nn.MaxPool1d(kernel_size=3, stride=3)

        self.submod2 = nn.Conv1d(in_channels=16, out_channels=num_classes, kernel_size=1, stride=1, padding='same')
        ...
        self.submod3 = nn.Conv1d(in_channels=32, out_channels=num_classes, kernel_size=1, stride=1, padding='same')

        # Layer 4
        # input [batch_size, 32, 83]  -->  afterconv&BN [batch_size, 32, 83]   -->  afterpool [batch_size, 32, 41]
        self.conv4 = nn.Conv1d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding='same')
        self.bn4 = nn.BatchNorm1d(32)
        self.pool4 = nn.MaxPool1d(kernel_size=2, stride=2)

        self.submod4 = nn.Conv1d(in_channels=32, out_channels=num_classes, kernel_size=1, stride=1, padding='same')

        # Layer 5
        # input [batch_size, 32, 41]  -->  afterconv&BN [batch_size, num_classess(16), 41]
        self.conv5 = nn.Conv1d(in_channels=32, out_channels=num_classes, kernel_size=1, stride=1, padding='same')
