...
if __name__ == "__main__":
    ...
    # Stopped midway as torch_tensorrt.compile does not understand some of the quant operator in the model
    
    acc_mod = acc_tracer.trace(quantized_net, example_inputs)
    #trt_model = torch_tensorrt.fx.compile(module=quantized_net, input=example_inputs)
    print(f"Elapsed time for 20% of dataset: {time.time() - start_time_inf:.2f} seconds")
    ########################################################################################################
