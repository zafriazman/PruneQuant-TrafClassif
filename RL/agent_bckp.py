...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        output_actions = []
        for eachlayer in state:
            action = self.relu(self.linear1(eachlayer))
            action = self.tanh(self.linear2(action))
            output_actions.append(action)
        actions = torch.tensor(output_actions)
...
class CriticNetwork(nn.Module):
    def __init__(self, in_size):
        ...
        self.linear1 = nn.Linear(in_size, 1)
        self.tanh = nn.Tanh()
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        self.to(self.device)
    ...
    def forward(self, state):
        ...
        values_list = []
        for eachlayer in state:
            value = self.tanh(self.linear1(eachlayer))
...
class Agent:
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            # rewards_expanded needed to match the dimension of the state
            """ rewards_expanded = rewards.unsqueeze(1).expand_as(state_values)
            ratios_expanded = ratios.unsqueeze(1).expand_as(state_values)
            advantages = rewards_expanded.detach() - state_values.detach()
            print(f"reward: {rewards_expanded}\nstate_val: {state_values}")
            print(f"ratios:{ratios}\nratios expanded:{ratios_expanded}")
            print(f"advantages: {advantages.shape}\n{advantages}")
            surr1 = ratios_expanded * advantages
            surr2 = torch.clamp(ratios, 1-self.eps_clip, 1+self.eps_clip) * advantages
            loss = -torch.min(surr1, surr2) + 0.5*self.MseLoss(state_values, rewards_expanded) - 0.01*dist_entropy
            if _ % 5 == 0:
                print(f"PPO epoch {_} \t loss: {loss.mean}") """
            # Finding Surrogate Loss:
