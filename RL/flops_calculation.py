...
def calc_perlayer_flops(orig_model, preserve_ratios):
    ...
    # Formula is (2 * {c_in * [c_out * preserve_ratio] * k_height * k_width} + [c_out * preserve_ratio]) * h_in_height * h_in_width
    ...
    input_perlayers = None
    flops_list = []
    i = 0

    if model.__class__.__name__ == "CNN1D_TrafficClassification":
        input_perlayers = [1500, 500, 166, 83, 41]  # either use torchsummary or manually calc based on pooling ops

    elif model.__class__.__name__ == "NiN_CNN1D_TrafficClassification":
        input_perlayers = [750, 750, 750, 375, 375, 375, 187, 187, 187, 187]
    ...
    for name, module in model.named_modules():
        ...
        if module.__class__.__name__ == 'Conv1d':
            ...
            h_in_height  = 1
            h_in_width = input_perlayers[i]

            layer_flops = (2 * (c_in * (c_out * preserve_ratios[i]) * k_height * k_width) + (c_out * preserve_ratios[i])) * h_in_height * h_in_width
