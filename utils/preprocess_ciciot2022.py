...
def preprocess_dataset():
    ...
    extracted_pcaps_dir = os.path.join('data', 'datasets', 'ciciot2022-extracted-pcaps')
    final_path = os.path.join('data', 'datasets', 'ciciot2022-pytorch')
    final_hash = {}
    random.seed(2024)
    

    if os.path.exists(os.path.join(final_path, '6-Attacks.pt')):
    ...
    class_naming = {
        ...
        '5-Active' : ['Active'],
        '6-Attacks' : ['Flood', 'BruteForce', 'RTSP', 'RSTP']
    }
    ...
    for class_name, data in final_hash.items():
        ...
        print(f"Counting number of packets takes {round(time.time() - start_time)}s for {class_name}")
