...
def preprocess_dataset():
    ...
    extracted_pcaps_dir = os.path.join('data', 'datasets', 'iscx2016vpn-extracted-pcaps')
    final_path = os.path.join('data', 'datasets', 'iscx2016vpn-pytorch')
    final_hash = {}
    random.seed(2024)
    

    if os.path.exists(os.path.join(final_path, 'youtube.pt')):
    ...
    # Extract .zip files code snippet
    # Better to unzip manually for more control on which pcap files to process
    # zip_files_directory = os.path.join('data', 'datasets', 'iscx2016vpn')
    # zip_files = [f for f in os.listdir(zip_files_directory) if f.endswith('.zip')]
    # for zip_file in zip_files:
    #     start_time = time.time()
    #     print("unzipping", zip_file)
    #     zip_file_path = os.path.join(zip_files_directory, zip_file)
    #     with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
    #         zip_ref.extractall(extracted_pcaps_dir)
    #     current_duration = round(time.time() - start_time)
    #     print(f"finish unzipping {zip_file} after {current_duration} seconds")


    # Easiest way to capture the filename without regex
    class_naming = {
        'aimchat': ['aim_chat', 'AIMchat'],
        'email' : ['email'],
        'facebook' : ['facebook'],
        'ftps' : ['ftps'],
        'gmail' : ['gmail'],
        'hangouts' : ['hangout', 'hangouts'],
        'icq' : ['icq', 'ICQ'],
        'netflix' : ['netflix'],
        'scp' : ['scp'],
        'sftp' : ['sftp'],
